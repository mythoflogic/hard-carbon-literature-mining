...
# RULE 10F
#
# Transposed multiword precursor / temperature-coded product
# family.
#
# Some source tables are transposed such that untreated raw
# precursors appear as full material names while their derived
# products use the precursor initials followed by treatment
# temperature:
#
#   Rice Husk   -> RH 300, RH 450, RH 600
#   Rice Straw  -> RS 300, RS 450, RS 600
#
# This is intentionally narrower than RULE 10E:
# - only TRANSPOSED_RECOVERED rows are considered;
# - precursor must contain at least two meaningful words;
# - precursor itself must have no numeric treatment temperature;
# - siblings must be in the SAME table;
# - sibling label must be exactly initials + 3-4 digit code,
#   allowing only space / hyphen / underscore separators;
# - encoded temperature must equal the sibling's recovered
#   treatment temperature;
# - at least three DISTINCT temperatures are required.
#
# No paper IDs, DOI values, benchmark identities, or benchmark
# temperatures are used.
# ============================================================

transposed_multiword_precursor_temp_na_indices = set()


def transposed_multiword_initials_07g2(
    sample,
):
...
for idx in (
    ...
    # Only rows created by the transposed elemental-table
    # recovery are eligible for this extension.
    if clean(
        row.get(
            "row_orientation"
        )
    ...
    # Existing exact treatment temperature wins.
    if numeric(
        row.get(
            "temperature_C"
        )
    ...
    family_prefix = (
        transposed_multiword_initials_07g2(
            sample
        )
    )
    ...
    for sibling_idx, sibling in (
        ...
        match = family_pattern.fullmatch(
            sibling_sample
        )
    ...
    # Same conservative repeated-family threshold as RULE 10E.
    if len(
        supported_temperatures
    ) < 3:
        continue

    transposed_multiword_precursor_temp_na_indices.add(
        idx
    )


temperature_not_applicable_indices.update(
    transposed_multiword_precursor_temp_na_indices
)

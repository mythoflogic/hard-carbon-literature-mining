...
def is_numeric_cell(value):
    ...
    # Markdown conversion can wrap the uncertainty marker
    # with emphasis underscores:
    #
    #     77.53_±_0.11
    #
    ...
    #     77.53±0.11
    #
    # Normalize only underscores surrounding the ± marker.
    # Do not remove arbitrary underscores from nonnumeric text.
    text = re.sub(
        r"_+\s*±\s*_+",
        "±",
        text,
...
def identify_element_columns(
    ...
    # --------------------------------------------------------
    # Reject Freundlich adsorption-model parameter tables.
    #
    # A header such as
    #
    #     Biochar | KF | n | R²
    #
    # uses lowercase n as the Freundlich exponent, not elemental
    # nitrogen. Keep the normal "n" nitrogen alias elsewhere,
    # but reject this characteristic parameter combination.
    # --------------------------------------------------------
    ...
    has_kf_parameter = any(
        part == "kf"
        for part in normalized_header_parts
    )
...
for record in candidate_records:
    ...
    # --------------------------------------------------------
    # Narrow 07D2 -> 07D3 structural-row wiring.
    #
    # Use structurally repaired rows only when 07D2 actually
    # exploded one merged source row into multiple samples.
    #
    # Do NOT activate this for generic uncertainty moves,
    # name merges, or figure-row removal because those changes
    # can alter unrelated table structure.
    # --------------------------------------------------------
    ...
    if multi_sample_explosions > 0:
        ...
        if (
            ...
            record[
                "_structural_header_rows_07d3"
            ] = [
                structural_header
            ]

            record[
                "_structural_data_rows_07d3"
            ] = structural_rows
...
for record in candidate_records:
    ...
    if (
        ...
        # 07D2 has already established the structural
        # header/data boundary. Preserve it rather than
        # reclassifying repaired PPS rows as headers.
        header_rows = [
            row
        ...
        data_rows = [
            row
            for row in structural_data_rows
            if (
                isinstance(
                    row,
                    list,

...
5. resolved_temperature_C must be ONE numeric value or null.
   Never place a range, list, string, or object in this field.
6. If the source supports a bounded processing-temperature range rather
   than one exact value, set resolved_temperature_C to null and return
   the numeric bounds in resolved_temperature_low_C and
   resolved_temperature_high_C.
7. Never convert a reported range to its midpoint, lower bound, or
   upper bound merely to create an exact temperature.
8. If resolved_temperature_C is numeric, both range-bound fields must
   be null. If a range is returned, both bounds must be numeric and
   low must be <= high.
9. resolved_sample must be one string or null.
   Never return an object.
10. Copy recovery_id EXACTLY as supplied.
11. Map slow pyrolysis and fast pyrolysis to "pyrolysis".
12. If the target row contains an exact temperature directly, prefer
    that value over temperatures mentioned elsewhere in the table.
13. If a sample code contains a clear temperature suffix such as PHC500
...
def build_user_prompt(package):
    expected_schema = {
        ...
        "resolved_temperature_low_C": None,
        "resolved_temperature_high_C": None,
...
def validate_result(
    ...
    for field_name, value in (
        ("resolved_temperature_C", temperature),
        ("resolved_temperature_low_C", range_low),
        ("resolved_temperature_high_C", range_high),
    ):
        ...
        if (
            ...
            raise ValueError(
                f"{field_name} outside plausible range"
            )
    ...
    if (
        ...
        raise ValueError(
            "Temperature range requires both bounds"
        )
    ...
    if (
        ...
        raise ValueError(
            "Exact temperature and range cannot "
            "both be populated"
        )
    ...
    if (
        ...
        raise ValueError(
            "Temperature range has low > high"
        )
    ...
    if (
        ...
        result["resolved_temperature_low_C"] = None
        result["resolved_temperature_high_C"] = None

...
sem_low = numeric(
    sem["resolved_temperature_low_C"]
)

sem_high = numeric(
    sem["resolved_temperature_high_C"]
)
...
if (
    ...
    raise RuntimeError(
        "Semantic temperature range has only "
        "one bound."
    )
...
if (
    ...
    raise RuntimeError(
        "Semantic temperature range has "
        "low > high."
    )
...
if (
    ...
    raise RuntimeError(
        "Semantic row contains both exact "
...
range_final = pd.DataFrame(
    {
        # Range rows come directly from the current 07G2
        # structural semantic queue. recovery_id is inherited
        # from an earlier queue and can collide with curated
        # semantic recovery IDs after queue regeneration.
        #
        # candidate_id_07g2 is the current branch-local ID and
        # uses the REC2_* namespace, so it is unique from the
        # semantic REC_* namespace. source_key remains the
        # stable provenance identity.
        "final_record_id":
            ranges["candidate_id_07g2"],
